    ...
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW:
            if event.from_user:
                ...
                us_key = str(us_id)
                if us_key not in active_users.keys():
                    active_users[us_key] = {}
                    active_users[us_key]['first_stage'] = False
                    active_users[us_key]['second_stage'] = False
                    active_users[us_key]['third_stage'] = False
                    active_users[us_key]['forth_stage'] = False
                    active_users[us_key]['counter'] = 0
                    active_users[us_key][menu_curr] = 'Main'

                favorites_list = db.get_favorite(us_id)
                favorites = logic.disable_favorites_button(favorites_list)
                if not active_users[us_key]['first_stage']:
                    ...
                    if not db.check_user(us_id):
                        ...
                        user_info = request.get_user(us_id)
                        active_users[us_key]['third_stage'] = True
                    ...
                    # active_users[us_key]['first_stage'] = first_stage
                    active_users[us_key]['first_stage'] = True
                elif event.obj.message['text'] == 'Закончить сеанс':
                    ...
                    us_key = str(us_id)
                    active_users[us_key]['first_stage'] = True
                    active_users[us_key]['second_stage'] = True
                if active_users[us_key]['first_stage'] and active_users[us_key]['second_stage'] \
                        and not active_users[us_key]['third_stage']:
                    us_id = logic.get_user_id(event)
                    user_info = request.get_user(us_id)
                    if 'bdate' not in user_info.keys() \
                            or not re.match(r'\d{1,2}\.\d{1,2}\.\d{2,4}$', user_info['bdate']) \
                            or 'city' not in user_info.keys() or 'sex' not in user_info.keys() or user_info['sex'] == 0:
                        us_id = logic.get_user_id(event)
                        user_info = request.get_user(us_id)
                        kb = keyboard.pers_data_permission_keyboard()
                        if event.obj.message['text'] != 'Согласен(на)':
                ...
                us_id = logic.get_user_id(event)
                us_key = str(us_id)
                if event.obj.message['text'] != '' and active_users[us_key]['first_stage']\
                        and active_users[us_key]['second_stage'] and active_users[us_key]['third_stage']:
                    if not active_users[us_key]['forth_stage']:
                        ...
                        active_users[us_key]['forth_stage'] = True
                    else:
                        ...
                        if vk.messages.getHistory(user_id=us_id, count=2)['items'][1]['text'] \
                            ...
                            db.update_hometown(us_id, city)
                                ...
                                ['Вперед', 'Назад', 'Начать поиск', 'Просмотреть избранное', 'В избранное',
                                 'В черный список']:
                        ...
                        # Сбрасываем все флаги, если пользователь нажал "Окончить сеанс".
                            ...
                            if event.obj.message['text'] == 'Начать поиск':
                                menu_curr = 'New_search'
                                us_id = logic.get_user_id(event)
                                us_key = str(us_id)
                                active_users[us_key]['menu_curr'] = menu_curr
                                if settings_change:
                                ...
                                if db.check_user(us_id) is False:
                                    ...
                                    try:
                                        ...
                                        res_source[us_id] = logic.provide_search_results(user_params=user_info,
                                                                                         user_token=read_token(),
                                                                                         offset=offset)
                                        print(f'Ошибка БД: {exs}')
                            # Готовим данные по id попавшим в "Избранное".
                            if event.obj.message['text'] == 'Просмотреть избранное':
                                menu_curr = 'Favorites'
                                us_id = logic.get_user_id(event)
                                us_key = str(us_id)
                                active_users[us_key]['menu_curr'] = menu_curr
                            ...
                            if event.obj.message['text'] == 'В черный список':
                                us_id = logic.get_user_id(event)
                                us_key = str(us_id)
                                counter = int(active_users[us_key]['counter'])
                            ...
                            if us_id in res_source.keys():
                                if event.obj.message['text'] in ['Начать поиск', 'Просмотреть избранное']:
                                    counter = 0
                                    active_users[us_key]['counter'] = counter
                                elif event.obj.message['text'] == 'Вперед':
                                    us_id = logic.get_user_id(event)
                                    us_key = str(us_id)
                                    if us_key in active_users.keys():
                                        counter = int(active_users[us_key]['counter'])
                                    if counter >= 0:
                                        if to_bl_trigger:
                                            to_bl_trigger = False
                                        elif forward_trigger:
                                            forward_trigger = False
                                            counter += 1
                                        else:
                                            counter += 1
                                ...
                                us_id = logic.get_user_id(event)
                                pos = active_users[us_key]['counter']
                                curr = active_users[us_key]['menu_curr']
                                ...
                                if event.obj.message['text'] not in ['В избранное', 'В черный список']:
                                    us_id = logic.get_user_id(event)
                                    keyboard.send_match_photos(vk, us_id, matches,
                                                               counter=active_users[us_key]['counter'])
        ...
        if event.type == VkBotEventType.MESSAGE_EVENT:
            ...
            # Предоставляем callback-кнопки и корректируем параметры фильтров поиска в БД.
                ...
                if event.object.payload.get("type") == "age_from_up":
                    us_id = logic.get_user_id(event)
                    db_response = db.increase_age_from(us_id)
                    settings_change = True
                elif event.object.payload.get("type") == "age_from_down":
                    us_id = logic.get_user_id(event)
                    db_response = db.reduce_age_from(us_id)
                    settings_change = True
                elif event.object.payload.get("type") == "age_to_up":
                    us_id = logic.get_user_id(event)
                    db_response = db.increase_age_to(us_id)
                    settings_change = True
                elif event.object.payload.get("type") == "age_to_down":
                    us_id = logic.get_user_id(event)
                    db_response = db.reduce_age_to(us_id)
                    settings_change = True
                elif event.object.payload.get("type") == "status_not_married":
                    us_id = logic.get_user_id(event)
                    db_response = db.update_status_not_married(us_id)
                    settings_change = True
                elif event.object.payload.get("type") == "status_in_search":
                    us_id = logic.get_user_id(event)
                    db_response = db.update_status_to_in_active_search(us_id)
                    settings_change = True
                else:
                    db_response = True
